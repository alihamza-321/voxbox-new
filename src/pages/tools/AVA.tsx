...
const AVA = () => {
  ...
  const getSessionStorageKey = (workspaceId: string | undefined) => {
    return workspaceId ? `ava-session-state-${workspaceId}` : 'ava-session-state';
  };
  ...
  const getInitialState = () => {
    ...
    try {
      ...
      if (workspaceId) {
        ...
        try {
          ...
          if (savedSession) {
            ...
            if (parsed.sessionId && parsed.currentStage && parsed.currentStage !== 'welcome') {
      ...
      const sortedKeys = keys.sort((a, b) => {
        ...
        const aIsWorkspaceScoped = a.startsWith('ava-session-state-') && a !== 'ava-session-state';
        const bIsWorkspaceScoped = b.startsWith('ava-session-state-') && b !== 'ava-session-state';
  ...
  const [currentStage, setCurrentStage] = useState<"welcome" | "phase1" | "phase2" | "complete">(initialState.currentStage);
  ...
  const [phase1Progress, setPhase1Progress] = useState<{ currentQuestionIndex: number; totalQuestions: number }>({ currentQuestionIndex: 0, totalQuestions: 27 });
  const [phase2Progress, setPhase2Progress] = useState<{ currentQuestionIndex: number; totalQuestions: number }>({ currentQuestionIndex: 0, totalQuestions: 0 });
  ...
  useEffect(() => {
    ...
    if (sessionId && initialSessionIdRef.current === sessionId && !hasAttemptedRestore.current) {
      // This is the session we initialized with - validate it belongs to this workspace
      getSessionDetails(sessionId)
        .then(session => {
          if (session.workspaceId !== workspaceId) {
            console.log(`⚠️ Initial session ${sessionId} belongs to workspace ${session.workspaceId}, but current workspace is ${workspaceId}. Resetting.`);
            setSessionId(null);
            setUserName("");
            setCurrentStage("welcome");
            setShowWelcome(true);
    ...
    if (hasAttemptedRestore.current !== null && hasAttemptedRestore.current !== workspaceId) {
      console.log(`🔄 Workspace changed from ${hasAttemptedRestore.current} to ${workspaceId}, clearing old session`);
      setSessionId(null);
      setUserName("");
      setCurrentStage("welcome");
      setShowWelcome(true);
    ...
    if (hasAttemptedRestore.current === workspaceId && sessionId) {
      ...
      getSessionDetails(sessionId)
        .then(session => {
          if (session.workspaceId !== workspaceId) {
            console.log(`⚠️ Session ${sessionId} belongs to workspace ${session.workspaceId}, but current workspace is ${workspaceId}. Clearing session.`);
            setSessionId(null);
            setUserName("");
            setCurrentStage("welcome");
            setShowWelcome(true);
    ...
    const restoreSession = async () => {
      try {
        ...
        if (savedSession) {
          ...
          if (parsed.sessionId && parsed.currentStage && parsed.currentStage !== 'welcome') {
            console.log(`✅✅✅ FOUND saved session for workspace ${workspaceId}:`, parsed);
            console.log(`✅ Restoring: stage=${parsed.currentStage}, sessionId=${parsed.sessionId}`);
            ...
            try {
              ...
              if (sessionDetails.workspaceId !== workspaceId) {
                console.log(`⚠️ Session ${parsed.sessionId} belongs to workspace ${sessionDetails.workspaceId}, but current workspace is ${workspaceId}. Not restoring.`);
              ...
              console.log(`✅✅✅ STATE RESTORED: stage=${parsed.currentStage}, sessionId=${parsed.sessionId}`);
            } catch (err: any) {
              console.error('❌ Failed to validate session workspace:', err);
            ...
            console.log(`⚠️ Invalid saved session for workspace ${workspaceId}`);
          }
        } else {
  ...
  useEffect(() => {
    ...
    if (sessionId && currentStage !== 'welcome') {
      ...
      console.log(`💾💾💾 Session state SAVED for workspace ${currentWorkspace.id}:`, sessionState);
    }
  ...
  useEffect(() => {
    const validateOrRecreateSession = async () => {
      ...
      try {
        ...
        const forbidden = msg.toLowerCase().includes('access denied') || msg.toLowerCase().includes('forbidden');
        if (forbidden) {
          if (!currentWorkspace?.id) return;
          try {
            const newSession = await createAvaSession(currentWorkspace.id, 'AVA Ideal Client Profile');
            const newId = newSession.id || newSession.sessionId;
            if (newId) {
              setSessionId(newId);
              // save immediately to localStorage (workspace-scoped)
              const storageKey = getSessionStorageKey(currentWorkspace.id);
              const sessionState = { sessionId: newId, userName: '', currentStage: 'phase1' };
              localStorage.setItem(storageKey, JSON.stringify(sessionState));
              toast({ title: 'New Session Created', description: 'Your previous session was not accessible. A new one has been created.' });
            }
          } catch (createErr: any) {
            toast({ title: 'Session Error', description: createErr?.message || 'Failed to create new session', variant: 'destructive' });
          }
        }
      }
    };
  ...
  const handlePhase2Complete = (_sections: any[]) => {
    ...
    if (currentWorkspace?.id && sessionId) {
      ...
      localStorage.setItem(storageKey, JSON.stringify(sessionState));
      console.log(`💾💾💾 Phase 2 Complete - keeping stage as phase2 to preserve content visibility`);
    }
  };
  ...
  const handleStartNew = async () => {
    ...
    if (currentWorkspace?.id) {
      ...
      if (sessionId) {
        localStorage.removeItem(`ava-phase1-state-${currentWorkspace.id}-${sessionId}`);
        localStorage.removeItem(`ava-phase2-${currentWorkspace.id}-${sessionId}`);
      }
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      console.log('🔄 Activate Now clicked - clearing any previous state to start fresh');
      ...
      console.log('📝 Backend session message type:', typeof backendSession.message, Array.isArray(backendSession.message) ? 'array' : typeof backendSession.message);
      ...
      if (Array.isArray(responseMessages) && responseMessages.length > 0) {
        console.log('✅ Storing', responseMessages.length, 'Activate Now messages to render as single chunk');
        // Store in sessionStorage for current session
        sessionStorage.setItem('activate-now-messages', JSON.stringify(responseMessages));
        ...
        if (currentWorkspace?.id) {
          ...
          localStorage.setItem(activateNowKey, JSON.stringify({
            messages: responseMessages,
            sessionId: newSessionId,
            timestamp: Date.now(),
          }));
          console.log('💾 Activate Now messages stored in localStorage for persistence:', activateNowKey);
        }
        
        console.log('✅ Activate Now messages stored in both sessionStorage and localStorage, ready for single chunk rendering');
      } else {
        console.warn('⚠️ No messages array in API response. Message value:', responseMessages);
        console.warn('⚠️ Backend session object keys:', Object.keys(backendSession));
      }
      ...
      if (currentWorkspace?.id) {
        ...
        localStorage.setItem(storageKey, JSON.stringify(sessionState));
        console.log(`💾💾💾 New session saved: Phase 1 for workspace ${currentWorkspace.id}`);
      }
      
      toast({
        title: "Session Created",
        description: "AVA is ready to help you create your ideal client profile",
      });
  ...
  const isPhase1Completed = useMemo(() => {
    return currentStage === "phase2" || currentStage === "complete" || (userName && currentStage !== "welcome");
  ...
  const handlePhase1Complete = (completedUserName: string) => {
    ...
    startTransition(() => {
      ...
      if (currentWorkspace?.id && sessionId) {
        ...
        localStorage.setItem(storageKey, JSON.stringify(sessionState));
        console.log(`💾💾💾 Phase transition saved: Phase 1 → Phase 2 for workspace ${currentWorkspace.id}`);
      }
    });
  ...
  const shouldShowWelcome = showWelcome && (!sessionId || currentStage === 'welcome');
  
  // Show welcome screen only if we don't have a valid session
  if (shouldShowWelcome) {
    return <AVAWelcome onStart={handleStartSession} isLoading={isCreatingSession} />;
  }
  ...
  const handlePhase2Export = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `AVA-Profile-${userName || 'Client'}-${new Date().toISOString().split('T')[0]}.pdf`;
      a.click();
  ...
  if (sessionId) {
    ...
    const headerStage = isPhase2Complete ? "complete" : (currentStage === "phase2" ? "phase2" : "phase1");
    const headerProgress = (currentStage === "phase2" || isPhase2Complete)
      ? phase2Progress 
