...
import { Send, ChevronDown, Edit, Check, X, Loader2, AlertCircle } from "lucide-react";
...
interface AVAPhase1InterfaceProps {
  ...
  onProgressChange?: (progress: { currentQuestionIndex: number; totalQuestions: number }) => void; // Report progress to parent for header
...
export const AVAPhase1Interface = ({
  ...
  const [isTyping, setIsTyping] = useState(() => {
    ...
    if (typeof window !== 'undefined' && sessionId) {
      const activateInProgress = sessionStorage.getItem('activate-now-in-progress');
      const activateSessionId = sessionStorage.getItem('activate-now-session-id');
  ...
  const [currentQuestion, setCurrentQuestion] = useState<Phase1Question | null>(null);
  const [progress, setProgress] = useState({ current: 0, total: 27, percentage: 0 });
  ...
  const handleNameSubmitRef = useRef<((name: string) => Promise<void>) | null>(null);
  ...
  const resetNameResponseStore = useAvaNameResponseStore(state => state.reset);
  
  const resetNameSubmissionProgress = (
    options: { preserveName?: boolean; preserveSession?: boolean; preserveStore?: boolean } = {}
  ) => {
      ...
      if (storedData) {
        ...
        if (parsed.messages && Array.isArray(parsed.messages) && parsed.messages.length > 0) {
          ...
          setMessages(prev => {
            ...
            const hasActivateMessage = prev.some((msg) => 
              msg.id?.includes('activate-msg') || 
              (msg.role === 'ava' && msg.content?.includes(combinedActivateMessage.substring(0, 50)))
            );
            
            if (hasActivateMessage) {
              console.log('ℹ️ Activate Now message already exists in messages, skipping restore');
              return prev;
            }
            ...
            if (parsed.timestamp) {
              ...
              if (prev.length > 0 && prev[0]?.timestamp) {
                const firstTimestamp = typeof prev[0].timestamp === 'string' 
                  ? new Date(prev[0].timestamp) 
                  : prev[0].timestamp;
                activateMessageTimestamp = new Date(firstTimestamp.getTime() - 60000);
              } else {
            ...
            console.log('💾 Restoring Activate Now message from localStorage immediately - prepending to top');
  ...
  const resumeNameSubmissionAfterRefresh = async () => {
    ...
    const nameSubmissionInProgressFlag = sessionStorage.getItem('name-submission-in-progress');
    const storeState = useAvaNameResponseStore.getState();
    const isSubmitNameResume = nameSubmissionInProgressFlag === 'true' || 
    ...
    if (isSubmitNameResume) {
      console.log('💾 Resuming Submit Name API - restoring Activate Now messages immediately');
    ...
    if (
      ...
      console.log('🔁 Resuming name submission via Zustand store, re-calling API');
    ...
    const storedNameSessionId = sessionStorage.getItem('name-submission-session-id');
    ...
    if (
      ...
      console.log('🔁 Resuming name submission via sessionStorage (API re-call)');
  ...
  const setAwaitingQuestionState = (state: AwaitingQuestionState) => {
    ...
    window.sessionStorage.setItem(awaitingQuestionKey, JSON.stringify({
      ...state,
      timestamp: Date.now(),
    }));
  };
  ...
  const loadPersistedState = async () => {
    if (!currentWorkspace?.id || !sessionId) {
      console.log('⚠️ Cannot load persisted state: missing workspace or sessionId');
      return null;
    }
    ...
    while (retryCount < maxRetries && !backendHistory) {
      try {
        console.log(`📡 Attempting to load conversation history from backend (attempt ${retryCount + 1}/${maxRetries})...`);
        const result = await getConversationHistory(sessionId);
        
        if (result && result.messages && result.messages.length > 0) {
          backendHistory = result;
          console.log(`✅ Successfully loaded ${result.messages.length} messages from backend`);
          break;
        } else {
          console.log(`⚠️ Backend returned empty messages array (attempt ${retryCount + 1})`);
        ...
        console.warn(`⚠️ Failed to load from backend (attempt ${retryCount + 1}/${maxRetries}):`, error);
        if (retryCount < maxRetries - 1) {
          // Wait before retrying (exponential backoff)
          await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
        }
    ...
    if (backendHistory && backendHistory.messages && backendHistory.messages.length > 0) {
      console.log('📂 Restoring Phase 1 chat from backend:', backendHistory.messages.length, 'messages');
      ...
      const nameQuestionIndex = backendHistory.messages.findIndex(
        (msg: any) => msg.role === 'ava' && msg.content?.includes("What's your name")
      );
      if (nameQuestionIndex >= 0 && nameQuestionIndex + 1 < backendHistory.messages.length) {
      ...
      const lastQuestion = [...backendHistory.messages].reverse().find(
        (msg: any) => msg.isQuestion && msg.role === 'ava'
      );
      ...
      if (lastQuestion) {
        ...
        if (lastQuestion.questionNumber) {
          extractedProgress = {
            ...
            percentage: Math.round(((lastQuestion.questionNumber) / (lastQuestion.totalQuestions || 27)) * 100),
          };
        }
        
        // Check if there's a user answer after the last question
        const lastQuestionIndex = backendHistory.messages.findIndex((msg: any) => msg.id === lastQuestion.id);
        const hasAnswerAfter = lastQuestionIndex >= 0 && 
    ...
    console.log('⚠️ Backend has no conversation history, trying localStorage fallback...');
    ...
    try {
      ...
      if (saved) {
        ...
        console.log('📂 Restoring Phase 1 chat state from localStorage:', parsed);
        ...
        if (parsed.messages && parsed.messages.length > 0 && sessionId) {
          ...
          try {
            ...
            console.log('✅ Successfully synced conversation history to backend');
          } catch (syncError) {
            console.warn('⚠️ Failed to sync conversation history to backend:', syncError);
  ...
  useEffect(() => {
    if (stage === "questions" || stage === "complete" || stage === "showing-intro") {
  ...
  useEffect(() => {
    ...
    const questionMessages = messages.filter(msg => msg.isQuestion && msg.questionNumber);
    ...
    if (currentQuestionNumber && currentQuestionNumber !== lastQuestionNumberRef.current) {
  ...
  useEffect(() => {
    ...
    const messageCountIncreased = currentMessageCount > previousMessageCount.current;
    ...
    if (isRestoredSession.current && !isInitialLoadComplete.current) {
      ...
      setTimeout(() => {
        ...
        if (currentStage !== "processing-name" && currentStage !== "showing-intro") {
  ...
  const chunkedContentCheckInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const shouldDisableAutoScroll = (): boolean => {
    const currentStage = stageRef.current || stage;
    return currentStage === "processing-name" || currentStage === "showing-intro";
  };
      ...
      if (sessionId) {
        ...
        pendingSaveRef.current = setTimeout(async () => {
          try {
            ...
            console.warn('⚠️ Failed to auto-save conversation to backend:', error);
          }
  ...
  }, [messages, stage, userName, progress, videoUrl, currentQuestion, sessionId]);
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      if (timeSinceLastSave > 1000) {
        ...
        xhr.open('POST', `${API_BASE_URL}/ava-sessions/${sessionId}/conversation-history`, false);
  ...
  useEffect(() => {
    ...
    const workspaceChanged = prevWorkspaceIdRef.current && 
                             prevWorkspaceIdRef.current !== currentWorkspace.id;
    const sessionChanged = prevSessionIdRef.current && 
  ...
  useEffect(() => {
    ...
    if (persistedState && persistedState.messages && persistedState.messages.length > 0) {
        // Restore persisted state - ALL messages including intro, name, video, questions, answers
        console.log('📂 Restoring Phase 1 chat with', persistedState.messages.length, 'messages');
          ...
          try {
            ...
            if (storedData) {
              ...
              if (parsed.messages && Array.isArray(parsed.messages) && parsed.messages.length > 0) {
                ...
                console.log('💾 Found Activate Now messages in localStorage:', parsed.messages.length, 'messages - will prepend to restored messages');
              }
            }
          } catch (e) {
        ...
        if (activateMessages && activateMessages.length > 0) {
          ...
          const hasActivateMessage = persistedState.messages.some((msg: any) => 
            msg.id?.includes('activate-msg') || 
            (msg.role === 'ava' && msg.content?.includes(combinedActivateMessage.substring(0, 50)))
          );
          ...
          if (!hasActivateMessage) {
            ...
            if (activateNowTimestamp) {
              ...
              if (firstMessage?.timestamp) {
                const firstTimestamp = typeof firstMessage.timestamp === 'string' 
            ...
            console.log('✅ Prepending Activate Now message to restored messages');
          } else {
            console.log('ℹ️ Activate Now message already exists in restored messages, skipping prepend');
          }
        }
      ...
      setProgress(persistedState.progress || { current: 0, total: 27, percentage: 0 });
      ...
      if (persistedState.stage === 'questions' && deduplicatedMessages.length > 0) {
        ...
        if (isLastMessageUserAnswer) {
          ...
          const lastQuestionIndex = deduplicatedMessages.map((msg, idx) => 
          ...
          const hasQuestionAfterAnswer = lastQuestionIndex !== undefined && 
          ...
          if (!hasQuestionAfterAnswer) {
            console.log('🔄 Detected user answer without next question after restoration - loading next question...');
      ...
      if (persistedState.stage === "questions" || persistedState.stage === "complete" || persistedState.stage === "showing-intro") {
        isIntroSequenceComplete.current = true;
      }
        
        console.log('✅ Phase 1 chat fully restored -', deduplicatedMessages.length, 'unique messages (deduplicated from', persistedState.messages.length, ')');
    } else {
      // Fresh start - show intro message
      console.log('🆕 Starting fresh Phase 1 chat - no persisted state found');
      ...
      if (
        ...
        console.log('🔁 Resuming name submission rendering via Zustand store');
        const storedName = nameResponseState.userName || '';
        if (storedName) {
          setUserName(storedName);
          setMessages(prev => {
            const hasUserNameMessage = prev.some(msg => msg.role === 'user' && msg.content === storedName);
        ...
        setIsSubmitting(true);
        try {
          await showIntroMessagesFromAPI(nameResponseState.messages, nameResponseState.videoUrl);
        } finally {
          setIsSubmitting(false);
        }
        hasInitialized.current = true;
      ...
      if (currentWorkspace?.id && sessionId) {
        ...
        try {
          ...
          if (storedData) {
            ...
            if (parsed.messages && Array.isArray(parsed.messages) && parsed.messages.length > 0) {
              activateMessages = parsed.messages;
              console.log('💾 Found Activate Now messages in localStorage for session:', parsed.messages.length, 'messages');
            }
          }
        } catch (e) {
          console.warn('Failed to parse localStorage activate messages:', e);
      ...
      if (!activateMessages) {
        ...
        if (activateInProgress === 'true' && activateSessionId !== sessionId) {
          console.log('🔄 Page was refreshed - session ID mismatch, clearing sessionStorage state');
        ...
        if (activateMessagesStr && activateSessionId === sessionId) {
          try {
            ...
            if (Array.isArray(parsedMessages) && parsedMessages.length > 0) {
              ...
              console.log('📝 Found Activate Now messages in sessionStorage for session:', activateMessages.length, 'messages');
            }
          } catch (e) {
            console.warn('Failed to parse sessionStorage activate messages:', e);
          }
        }
      }
      ...
      if (activateMessages && activateMessages.length > 0) {
        ...
        const isFreshClick = activateInProgress === 'true' && activateSessionId === sessionId;
        const isRefresh = !isFreshClick;
        
        console.log('🎬 Rendering Activate Now messages:', activateMessages.length, 'messages');
        console.log(`   - activate-now-in-progress: ${activateInProgress}`);
        console.log(`   - activate-now-session-id: ${activateSessionId}`);
        console.log(`   - current sessionId: ${sessionId}`);
        console.log(`   - Mode: ${isRefresh ? 'REFRESH (restore immediately)' : 'FRESH CLICK (show typing)'}`);
  ...
  useEffect(() => {
    if (messages.length > 0 && !isSubmitting && stage !== "processing-name") {
      ...
      if (isTyping) {
        console.log('🔧 Safety check: Clearing typing indicator - messages exist and not processing');
        setIsTyping(false);
      }
    }
  ...
  const addMessage = async (message: MessageType, delay: number = 1000) => {
    ...
    setMessages(prev => {
      ...
      if (exists) {
        console.log('⚠️ Duplicate message detected, skipping:', message.content?.substring(0, 50));
        return prev;
      }
      return [...prev, message];
    });
    ...
    setMessages(prev => {
      const exists = prev.some(m =>
        ...
        (!message.allowDuplicateContent &&
          m.role === message.role &&
          m.content === message.content &&
          ((!m.videoUrl && !message.videoUrl) || m.videoUrl === message.videoUrl))
      );
      if (exists) {
        return prev;
      }
  ...
  const renderActivateNowMessages = async (messages: string[], isRefresh: boolean = false) => {
    console.log('🎬 Rendering Activate Now messages as single chunk:', messages.length, 'messages', isRefresh ? '(refresh restore)' : '(initial click)');
    ...
    if (isRefresh) {
      ...
      console.log('💾 Restoring Activate Now messages immediately (refresh scenario)');
      
      // Clear any existing messages with activate-msg prefix to avoid duplicates (only for refresh)
      setMessages(prev => {
        const filtered = prev.filter(msg => !msg.id?.startsWith('activate-msg-'));
        const activateMessageId = `activate-msg-restored-${sessionId}-${Date.now()}`;
        // Check if message already exists
        const exists = filtered.some(msg => 
          msg.role === 'ava' && msg.content === combinedMessage
        );
        if (exists) {
          console.log('ℹ️ Activate Now message already exists, skipping restore');
          return filtered;
        }
      ...
      console.log('⏳ Fresh click: Showing typing indicator FIRST, then rendering Activate Now messages');
      ...
      setMessages(prev => prev.filter(msg => !msg.id?.startsWith('activate-msg-')));
  ...
  const showIntroMessage = async (initialMessage?: string | null) => {
    if (initialMessage) {
      ...
      console.log('📝 Using initial message from API:', initialMessage.substring(0, 100));
      await addMessage({
      ...
      await addMessage({
        id: `msg-${Date.now()}`,
        role: 'ava',
        content: "Hi! I'm A.V.A. (Audience Values Accelerator), an elite-level AI specifically engineered to help online coaches, course creators, and consultants achieve unprecedented clarity about their ideal clients.",
        timestamp: new Date(),
      }, 1500);
      ...
      await addMessage({
        id: `msg-${Date.now()}`,
        role: 'ava',
        content: "I go far beyond typical audience profiling, delivering deep psychological and psychographic analysis. My insights uncover hidden motivations, subconscious desires, emotional barriers, and decision-making triggers, empowering you to resonate powerfully with your ideal clients.",
        timestamp: new Date(),
      }, 1000);
  ...
  const ensureUserNameMessage = (name: string, forceNew: boolean = false) => {
    setMessages(prev => {
      const hasUserNameMessage = prev.some(msg => msg.role === 'user' && msg.content === name);
  ...
  const executeNameSubmission = async (
    ...
    console.log('📤 Calling updateSessionName API for session:', sessionId, 'with name:', name);
    console.log('📤 API URL:', `${API_BASE_URL}/ava-sessions/${sessionId}/name`);

    const response: SubmitNameResponse = await updateSessionName(sessionId, name);
    ...
    if (messageArray && messageArray.length > 0) {
      try {
        ...
        console.warn('⚠️ Failed to persist name response to storage:', storageError);
        if (storageError.message?.includes('quota')) {
          console.warn('⚠️ Storage quota exceeded, continuing without persistence');
        }
      }
  ...
  const handleNameSubmit = async (name: string) => {
    console.log('🔄 Name submission started - clearing any previous state to start fresh from index 0');
    const isRetrigger = sessionStorage.getItem('name-submission-retrigger') === 'true';
    resetNameSubmissionProgress({ preserveName: isRetrigger, preserveSession: isRetrigger, preserveStore: true });
    if (isRetrigger) {
    ...
    setMessages(prev => prev.filter(msg => !msg.id?.startsWith('name-submit-msg-')));
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to submit name. Please check console for details.",
        variant: "destructive",
      });
  ...
  useEffect(() => {
    ...
    const storedSessionId = sessionStorage.getItem('name-submission-session-id');
    if (storedSessionId && storedSessionId !== sessionId) {
      console.log('⚠️ Stored name submission session mismatch. Clearing state.');
  ...
  const showIntroMessagesFromAPI = async (messages: string[], videoUrl?: string) => {
    console.log('🎬 Rendering name submission API response messages:', messages.length, 'messages');
    ...
    const hasCompletedPreviously = alreadyDisplayed >= totalMessages && totalMessages > 0;
    ...
    if (isResuming) {
      ...
      sessionStorage.setItem('name-submission-messages-displayed', String(startIdx));
    } else {
      sessionStorage.setItem('name-submission-messages-displayed', '0');
    }
    
    if (!hasCompletedPreviously) {
      await new Promise(resolve => setTimeout(resolve, isResuming ? 500 : 1000));
      ...
      for (let idx = startIdx; idx < totalMessages; idx++) {
        ...
        console.log(`✅ Rendered name submission message ${idx + 1}/${totalMessages}`);
    ...
    if (resolvedVideoUrl && !videoShown) {
      await addMessage({
        id: `msg-${Date.now()}`,
        role: 'ava',
        content: "Before we proceed, please watch this video to understand the process:",
        timestamp: new Date(),
        videoUrl: resolvedVideoUrl,
      }, 1000);
  ...
  const isAnswerSameAsQuestion = (answer: string, question: Phase1Question | null): boolean => {
    ...
    const questionWithNumber = stripHtmlAndNormalize(`${question.id}. ${question.text}`);
    ...
    const normalizedFormattedQuestion = stripHtmlAndNormalize(formattedQuestion);
  ...
  const handleAnswerSubmit = async (answer: string) => {
    ...
    if (isAnswerSameAsQuestion(answer, currentQuestion)) {
      setValidationError("Please provide your own answer. You cannot submit the question text as your answer.");
      // Also show toast for additional visibility
      toast({
        title: "Invalid Answer",
        description: "Please provide your own answer. You cannot submit the question text as your answer.",
        variant: "destructive",
      });
    ...
    if (lastQuestionMessage) {
      const normalizedAnswer = stripHtmlAndNormalize(answer);
      const normalizedQuestionContent = stripHtmlAndNormalize(lastQuestionMessage.content);
    ...
    try {
      ...
      if (response.isPhaseComplete || response.phase2AutoStarted || response.nextAction === 'start_phase2') {
        ...
        if (response.videoUrl) {
          await addMessage({
            id: `msg-${Date.now()}`,
            role: 'ava',
            content: "Phase 1 is now complete. Watch this video to learn about Phase 2:",
            timestamp: new Date(),
            videoUrl: response.videoUrl,
          }, 1200);
        }
        ...
        console.log('✅ Phase 1 complete - conversation history preserved for Phase 2');
          ...
          requestAnimationFrame(() => {
            ...
            if (startPhase2ButtonRef.current) {
              ...
              const scrollToButton = () => {
                ...
                const startPhase2Button = buttons.find(btn => btn.textContent?.trim().includes('Start Phase 2'));
                if (startPhase2Button) {
                  ...
                  console.log('📜 Scrolled to Start Phase 2 button (found by text)');
                  return true;
                }
                return false;
              };
        ...
        if (response.motivationalMessage) {
          ...
          setMessages(prev => {
            ...
            if (exists) {
              console.log('⚠️ Duplicate motivational message detected, skipping');
              return prev;
            }
            return [...prev, message];
          });
  ...
  const loadNextQuestion = async () => {
    ...
    try {
      ...
      const questionResponse = await fetch(`${API_BASE_URL}/ava-sessions/${sessionId}/current-question`, {
        method: 'GET',
        headers: {
      ...
      if (nextQ.question) {
        ...
        await addMessage({
          ...
          model: 'claude-sonnet-4-20250514',
        }, 1000); // 1 second delay before showing next question
        clearAwaitingQuestionState();
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (pendingQuestionRestoreRef.current || hasAttemptedQuestionRecovery.current) return;
    ...
    if (messages.length > 0) {
      ...
      // Find the last question in the messages
      const lastQuestionIndex = messages.map((msg, idx) => 
        msg.isQuestion && msg.role === 'ava' ? idx : -1
      ).filter(idx => idx >= 0).pop();
      
      // Check if there's a question after the last user answer
      const lastUserAnswerIndex = isLastMessageUserAnswer ? messages.length - 1 : -1;
      const hasQuestionAfterLastAnswer = lastQuestionIndex !== undefined && 
                                         lastQuestionIndex > lastUserAnswerIndex;
      ...
      if (isLastMessageUserAnswer && !hasQuestionAfterLastAnswer) {
        console.log('🔄 Detected user answer without next question - loading next question...');
    ...
    console.log('🔄 Detected pending question after refresh. Resuming fetch...');
    pendingQuestionRestoreRef.current = true;
    hasAttemptedQuestionRecovery.current = true;
    loadNextQuestionRef.current?.().finally(() => {
      pendingQuestionRestoreRef.current = false;
  ...
  const isAnswerMessage = (msg: MessageType, msgIndex: number): boolean => {
    ...
    if (msg.role !== 'user' || (stage !== 'questions' && stage !== 'complete')) {
      return false;
    }
    ...
    const hasQuestionBefore = messagesBeforeThis.some(m => m.isQuestion && m.role === 'ava');
    ...
    const isFirstUserMessage = !messagesBeforeThis.some(m => m.role === 'user');
    const isNameMessage = msg.isName === true || (!hasQuestionBefore && isFirstUserMessage);
    
    return hasQuestionBefore && !isNameMessage;
  };

  // Handle reset/new session
  const handleReset = onStartOver || (async () => {
  ...
  return (
    <div className={`flex flex-col ${hideHeader ? '' : 'min-h-screen'} pb-4 relative`}>
      {/* Chat Messages Area - Scrollable only when header is shown, otherwise parent handles scrolling */}
      <div 
        ref={scrollContainerRef}
        className={`${hideHeader ? '' : 'flex-1 overflow-y-auto'} px-[20%] py-8`}
      ...
      >
        ...
        <div className="w-full space-y-6">
          ...
          {messages.filter((msg, index, self) => {
            ...
            if (!msg.id) {
              const contentIndex = self.findIndex(m => 
                m.role === msg.role && 
            ...
            <div key={`${msg.id}-${msgIndex}`} data-question-id={msg.isQuestion ? msg.id : undefined}>
              {msg.role === 'ava' ? (
                /* AVA Message - Left Side */
                  ...
                  <div className="w-[70%]">
                    <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl rounded-tl-sm px-4 py-3">
                        {(() => {
                          ...
                          const isHTML = typeof msg.content === 'string' && /<[^>]+>/.test(msg.content);
                          return (
                            <div className={`text-[15px] leading-relaxed text-slate-200 ${isHTML ? '' : 'whitespace-pre-wrap'}`}>
                              {isHTML ? (
                                <div 
                        ...
                        {msg.videoUrl && (
                          <div className="mt-4">
                            ...
                            {stage === "showing-intro" && msg.content?.includes("Before we proceed, please watch this video") && (
                            ...
                            {stage === "complete" && !hideStartPhase2Button && msg.content?.includes("Phase 1 is now complete") && (
                              <div className="flex justify-center mt-6 animate-fade-in">
                                <Button
                                  ...
                                  sessionStorage.setItem('ava-phase2-transition-scroll', JSON.stringify(scrollPayload));
                                  sessionStorage.setItem('ava-phase2-hide-phase1-messages', 'true');

                                  requestAnimationFrame(() => {
                                    window.scrollTo({ top: scrollPayload.scrollY, left: scrollPayload.scrollX });
                                  });
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-700/50">
                            <button
                              onClick={() => {
                                ...
                                const isLastQuestion = messages.filter(m => m.isQuestion).pop()?.id === currentMsgId;
                                if (isLastQuestion) {
                            ...
                            >
                              <ChevronDown className={`w-3 h-3 transition-transform ${showExamples && messages.filter(m => m.isQuestion).pop()?.id === msg.id ? 'rotate-180' : ''}`} />
                              Show examples
                            </button>
                            {showExamples && messages.filter(m => m.isQuestion).pop()?.id === msg.id && (
                          ...
                          {editingMessageId === msg.id ? (
                            ...
                            <>
                              <div className="flex gap-3 items-end">
                                <div className={`bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-2xl rounded-tr-sm px-4 py-3 ${isNameMessage ? 'w-fit max-w-full' : 'w-full'}`}>
                                  {(() => {
                                    // Check if content is HTML (user messages typically aren't, but check anyway)
                                    const isHTML = typeof msg.content === 'string' && /<[^>]+>/.test(msg.content);
                                    return (
                                      <p className={`text-[15px] leading-relaxed text-slate-200 ${isHTML ? '' : 'whitespace-pre-wrap'}`}>
                                        {isHTML ? (
                                ...
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 text-white font-bold text-sm flex items-center justify-center flex-shrink-0 shadow-md shadow-cyan-500/30">
                                  <span>{getUserInitials(userName || msg.content)}</span>
                                </div>
                              </div>
                              ...
                              {isAnswerMessage(msg, msgIndex) && (
                                <div className="flex gap-2 items-center w-full justify-end">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => handleEditAnswer(msg.id, msg.content)}
          ...
          <div className="flex-1 relative flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <Textarea
                ...
                className={`flex-1 bg-slate-950/50 border ${
                  validationError ? 'border-red-500/50 focus:border-red-500/50' : 'border-slate-800 focus:border-cyan-500/50'
                } rounded-[7px] pl-4 pr-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 transition-all placeholder:text-slate-600 resize-none min-h-[40px] max-h-[120px] overflow-y-auto scrollbar-hide ${
            ...
            {validationError && (
              <div className="flex items-start gap-2 px-1 animate-fade-in">
                <div className="flex-1 bg-red-950/30 border-2 border-red-500/50 rounded-lg px-3 py-2.5 shadow-sm">
