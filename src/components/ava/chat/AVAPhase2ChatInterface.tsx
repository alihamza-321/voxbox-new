...
import { Edit, RefreshCw, Check, Loader2, Download, Sparkles, FileText } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface AVAPhase2ChatInterfaceProps {
  ...
  onStartOver?: () => void;
  onProgressChange?: (progress: { currentQuestionIndex: number; totalQuestions: number }) => void;
...
export const AVAPhase2ChatInterface = ({ 
  ...
  const loadPersistedState = (workspaceId: string | undefined) => {
    ...
    try {
      ...
      if (saved) {
        ...
        console.log(`📂 Restoring Phase 2 state (raw) for workspace ${workspaceId}:`, parsed);
        ...
        if (parsed?.sections && Array.isArray(parsed.sections)) {
          ...
          parsed.sections = parsed.sections.map((section: Section) => {
            console.log(`🧹 Sanitizing section ${section.sectionNumber} - resetting approval states and clearing stuck generating states`);
            return {
              ...section,
              questions: section.questions.map(q => ({
                ...
                status: q.status === 'generating' ? 'ready' as const : q.status,
              })),
            };
          });
        } else {
          // New minimal format: only restore position, backend will provide content
          console.log('📂 Detected minimal state format - will restore position only, backend will provide content');
        }
  ...
  const [sectionsList, setSectionsList] = useState<Array<{ sectionNumber: number; title: string; intro: string; isApproved?: boolean }>>([]);
  ...
  const [phase1History, setPhase1History] = useState<Array<{
    questionId: string;
    sectionId: string;
    questionText: string;
    answer: string;
  ...
  const [phase1IntroMessages, setPhase1IntroMessages] = useState<Array<{
    id: string;
    role: 'ava' | 'user';
    content: string;
    videoUrl?: string;
  ...
  const [showCreateNewProfileDialog, setShowCreateNewProfileDialog] = useState(false);
  ...
  useEffect(() => {
    ...
    try {
      const storedScrollData = sessionStorage.getItem('ava-phase2-transition-scroll');
      if (storedScrollData) {
        const parsed = JSON.parse(storedScrollData);
        const targetScrollY = typeof parsed.scrollY === 'number' ? parsed.scrollY : window.scrollY;
        const targetScrollX = typeof parsed.scrollX === 'number' ? parsed.scrollX : window.scrollX;

        window.scrollTo({ top: targetScrollY, left: targetScrollX, behavior: 'auto' });
        console.log('📜 Restored scroll position for Phase 2 transition:', { targetScrollY, targetScrollX });
  ...
  useEffect(() => {
    ...
    const restoreFromBackend = async () => {
      try {
        ...
        if (backendState && backendState.sections && backendState.sections.length > 0) {
          console.log('✅ Restoring Phase 2 state from backend:', backendState);
          // Sanitize persisted state (same as initial load)
          const sanitizedSections = backendState.sections.map((section: Section) => ({
            ...section,
          ...
          const restoredIndex = backendState.currentQuestionIndex !== undefined 
        ...
        console.warn('⚠️ Failed to restore from backend, falling back to localStorage:', error);
      }
      ...
      if (persistedState) {
        ...
        if (persistedState.sections && Array.isArray(persistedState.sections) && persistedState.sections.length > 0) {
          ...
          console.log('📂 Initial load - restoring Phase 2 state from localStorage (old format) for workspace:', currentWorkspace.id);
          const sanitizedSections = persistedState.sections.map((section: Section) => ({
            ...section,
            questions: section.questions.map((q: any) => ({
              ...q,
              isApproved: false,
          ...
          const restoredIndex = persistedState.currentQuestionIndex !== undefined 
          ...
          console.log('✅ Phase 2 state restored from localStorage (old format)', { 
            section: persistedState.currentSection, 
            questionIndex: restoredIndex 
          });
        } else if (persistedState.currentSection !== undefined) {
          // New minimal format: only restore position, backend will provide content
          console.log('📂 Initial load - restoring Phase 2 position from localStorage (minimal format) for workspace:', currentWorkspace.id);
          setCurrentSection(persistedState.currentSection || null);
          // Preserve the exact question index from localStorage - don't default to 0
          const restoredIndex = persistedState.currentQuestionIndex !== undefined 
            ? persistedState.currentQuestionIndex 
          ...
          restoredQuestionIndexRef.current = restoredIndex; // Track restored index
          setIsStarted(persistedState.isStarted || false);
          console.log('✅ Phase 2 position restored from localStorage (minimal format)', { 
            section: persistedState.currentSection, 
            questionIndex: restoredIndex 
          });
        ...
        if (phase1History.length === 0 && backendSessionId) {
          (async () => {
            try {
              ...
              console.log(`✅ Restored ${backendAnswers.length} Phase 1 answers from backend`);
              ...
              setPhase1History(sortedAnswers.length > 0 ? sortedAnswers : backendAnswers);
            } catch (error) {
              console.warn('⚠️ Failed to load Phase 1 history during localStorage restore:', error);
            }
          })();
        }
        ...
        if (!hasLoadedPhase1Messages.current && currentWorkspace?.id && backendSessionId) {
            ...
            try {
              ...
              if (backendHistory.messages && backendHistory.messages.length > 0) {
                ...
                backendHistory.messages.forEach((msg: any) => {
                  const key = msg.id || `${msg.role}-${msg.content?.substring(0, 50)}-${msg.videoUrl || ''}`;
                ...
                console.log(`✅ Loaded ${uniqueMessages.length} unique Phase 1 messages from backend (deduplicated from ${backendHistory.messages.length})`);
              } else {
                ...
                if (phase1State) {
                  ...
                  if (parsed.messages && Array.isArray(parsed.messages)) {
                    ...
                    setPhase1IntroMessages(uniqueMessages);
                    console.log(`✅ Loaded ${uniqueMessages.length} unique Phase 1 messages from localStorage (deduplicated from ${parsed.messages.length})`);
                  }
                } else {
                  console.log('⚠️ No Phase 1 messages found in backend or localStorage');
                }
              }
            } catch (error) {
        ...
        console.log('📂 Initial load - no saved Phase 2 state for workspace:', currentWorkspace.id);
  ...
  useEffect(() => {
    if (!currentWorkspace?.id || !backendSessionId || hasLoadedPhase1Messages.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (workspaceChanged || sessionChanged) {
      ...
      console.log('🔄 Workspace/session changed, validating and reloading Phase 2 state');
      
      // CRITICAL: When workspace changes, immediately reset state to prevent showing wrong workspace's data
      if (workspaceChanged) {
        console.log(`🔄 Workspace changed from ${previousWorkspaceId} to ${currentWorkspace.id}, resetting Phase 2 state`);
        setSections([]);
      ...
      const validateAndLoadState = async () => {
        try {
          ...
          if (sessionDetails.workspaceId !== currentWorkspace.id) {
            console.log(`⚠️ Session ${backendSessionId} belongs to workspace ${sessionDetails.workspaceId}, but current workspace is ${currentWorkspace.id}. Not loading Phase 2 state.`);
          ...
          if (newPersistedState) {
            // Handle both old format (with sections) and new minimal format
            if (newPersistedState.sections && Array.isArray(newPersistedState.sections) && newPersistedState.sections.length > 0) {
              // Old format: restore full sections
              console.log('🔄 Workspace/session changed - reloading Phase 2 state (old format):', newPersistedState);
              const sanitizedSections = newPersistedState.sections.map((section: Section) => ({
                ...section,
                questions: section.questions.map((q: any) => ({
                  ...q,
                  isApproved: false,
              ...
              setCurrentQuestionIndex(newPersistedState.currentQuestionIndex || 0);
              ...
              console.log('🔄 Workspace/session changed - reloading Phase 2 position (minimal format):', newPersistedState);
              setCurrentSection(newPersistedState.currentSection || null);
              setCurrentQuestionIndex(newPersistedState.currentQuestionIndex || 0);
              setIsStarted(newPersistedState.isStarted || false);
              // Sections will be loaded from backend
            ...
            console.log('🔄 Workspace/session changed - no saved Phase 2 state, ensuring reset');
            setSections([]);
            setCurrentSection(null);
            setCurrentQuestionIndex(0);
            setSectionsList([]);
          ...
          console.error('❌ Failed to validate session workspace in Phase 2:', err);
  ...
  useEffect(() => {
    if (currentSection && sectionScrollRef.current) {
      const sectionElement = sectionScrollRef.current.querySelector(`[data-section-number="${currentSection}"]`);
      if (sectionElement) {
        sectionElement.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
      }
    }
  }, [currentSection]);
  ...
  useEffect(() => {
    ...
    const hasAnyRestoredState = restoredQuestionIndexRef.current !== null || hasRestoredState.current;
    
    if (!hasAnyRestoredState) {
      // Fresh session - just scroll to bottom naturally, don't restore to a specific question
      console.log('🆕 Fresh session detected - skipping scroll restoration, will scroll naturally');
    ...
    const targetIndex = restoredQuestionIndexRef.current !== null 
    ...
    if (restoredQuestionIndexRef.current !== null) {
    ...
    } else if (normalizedIndex === 0 && currentSectionData.questions.length > 1 && hasRestoredState.current) {
      // Only try to find first unapproved question if we have restored state
      // This prevents unwanted behavior in fresh sessions
      const firstUnapprovedIndex = currentSectionData.questions.findIndex(q => !q.isApproved);
      if (firstUnapprovedIndex >= 0) {
        ...
        if (currentQuestionIndex !== firstUnapprovedIndex) {
          ...
          console.log(`📍 Updated currentQuestionIndex from ${currentQuestionIndex} to ${firstUnapprovedIndex} (first unapproved question)`);
        }
      }
    }
    ...
    if (currentQuestionIndex !== normalizedIndex) {
      ...
      console.log(`📍 Adjusting question index from ${currentQuestionIndex} to ${normalizedIndex} for scroll restoration`);
    }
    ...
    console.log(`📍 Attempting to restore scroll to question ${normalizedIndex + 1} in section ${currentSection}`, {
      anchorId,
      currentQuestionIndex,
      normalizedIndex,
    ...
    const attemptScroll = (attempt = 0) => {
      ...
      if (target) {
        requestAnimationFrame(() => {
          ...
          setTimeout(() => {
            ...
            console.log('✅ Restored scroll to Phase 2 question and scrolled to end', { 
              anchorId, 
              attempt,
      ...
      if (attempt < 20) {
        ...
        const timeoutId = window.setTimeout(() => attemptScroll(attempt + 1), delay * (attempt + 1));
        timeouts.push(timeoutId);
      } else {
        console.warn('⚠️ Failed to restore scroll after 20 attempts - question element not found', { anchorId });
  ...
  const getCurrentSectionProgress = () => {
    ...
    const approvedQuestionsCount = currentSectionData.questions.filter(q => q.isApproved).length;
  ...
  useEffect(() => {
    ...
    if (!currentWorkspace?.id || !backendSessionId || currentSection === null || !hasInitialized.current) {
      return;
    }
    ...
    const timeoutId = setTimeout(async () => {
      try {
        await savePhase2Progress(backendSessionId, currentSection, currentQuestionIndex);
        console.log(`💾 Phase 2 progress saved to backend: Section ${currentSection}, Question Index ${currentQuestionIndex}`);
      } catch (error: any) {
  ...
  }, [currentSection, currentQuestionIndex, backendSessionId, currentWorkspace?.id]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (sizeInMB > 4) {
        console.warn(`⚠️ Phase 2 state is too large (${sizeInMB.toFixed(2)}MB), saving minimal state only`);
      ...
      console.log(`💾 Phase 2 state saved to localStorage for workspace ${currentWorkspace.id}:`, { 
        currentSection, 
        currentQuestionIndex, 
      ...
      if (error.name === 'QuotaExceededError' || error.message?.includes('quota')) {
        console.warn('⚠️ localStorage quota exceeded, attempting to clear old Phase 2 data and save minimal state');
        ...
        try {
          ...
          for (let i = 0; i < localStorage.length; i++) {
            ...
            if (storageKey && storageKey.startsWith(`ava-phase2-${currentWorkspace.id}-`) && storageKey !== key) {
              keysToRemove.push(storageKey);
            }
          }
          keysToRemove.forEach(k => localStorage.removeItem(k));
          console.log(`🧹 Cleared ${keysToRemove.length} old Phase 2 localStorage entries`);
          ...
          console.error('❌ Failed to save Phase 2 state even after clearing old data:', retryError);
          // Don't throw - backend will handle state restoration
        }
      } else {
        console.error('❌ Failed to save Phase 2 state to localStorage:', error);
        // Don't throw - backend will handle state restoration
      }
    }
  ...
  }, [sections, currentSection, currentQuestionIndex, sectionsList, isStarted, currentWorkspace?.id, backendSessionId, phase1IntroMessages]);
  ...
  useEffect(() => {
    ...
    const shouldHidePhase1Messages = sessionStorage.getItem('ava-phase2-hide-phase1-messages') === 'true';
  ...
  useEffect(() => {
    ...
    if (chunksDisplaying === 'true') {
      console.log('🔄 Page was refreshed while chunks were displaying - re-triggering API from start');
    ...
    console.log('Full props received:', { backendSessionId, userName, phase1Answers });

    // Check if force reinit is needed (refresh during chunk rendering)
    const forceReinit = sessionStorage.getItem('phase2-force-reinit') === 'true';
    ...
    const checkBackendProgress = async () => {
      try {
        ...
        if (backendProgress && backendProgress.sections && backendProgress.sections.length > 0) {
          console.log('✅ Found existing Phase 2 progress in backend:', backendProgress);
          console.log(`   Current Section: ${backendProgress.currentSection}, Question Index: ${backendProgress.currentQuestionIndex}`);
          ...
          const sanitizedSections = backendProgress.sections.map((section: Section) => ({
            ...section,
            questions: section.questions.map((q: any) => ({
              ...q,
          ...
          const restoredIndex = backendProgress.currentQuestionIndex !== undefined 
          ...
          console.log(`✅ Restored Phase 2 state: Section ${restoredSection}, Question Index ${restoredIndex}`);
          ...
          if (phase1History.length === 0 && backendSessionId) {
            (async () => {
              try {
                ...
                console.warn('⚠️ Failed to load Phase 1 history during restore:', error);
              }
            })();
          }
          ...
          if (currentSectionNum && backendSessionId) {
            console.log(`🔄 Refreshing section ${currentSectionNum} status from backend...`);
              ...
              .then(statusResponse => {
                ...
                setSections(prev => prev.map(section => {
                  if (section.sectionNumber === currentSectionNum) {
                    const updatedQuestions = (sectionData?.questions || []).map((q: any) => ({
                      id: q.id,
                      questionId: q.questionId,
                      questionText: q.questionText,
                      aiGeneratedAnswer: q.aiGeneratedAnswer,
                      userEditedAnswer: q.userEditedAnswer,
                      status: q.status || (q.aiGeneratedAnswer ? 'ready' as const : 'generating' as const),
                      isApproved: q.isApproved || false,
                    }));
                    
                    // Ensure currentQuestionIndex is preserved and not reset
                    // Find the first unapproved question or use saved index
                    const firstUnapprovedIndex = updatedQuestions.findIndex((q: QuestionResponse) => !q.isApproved);
                    const targetIndex = firstUnapprovedIndex >= 0 
                      ? firstUnapprovedIndex 
                      : Math.min(savedQuestionIndex, updatedQuestions.length - 1);
                    
                    // Update question index if needed to ensure we're at the right position
                    if (targetIndex !== savedQuestionIndex && targetIndex >= 0) {
                      console.log(`📍 Adjusting question index from ${savedQuestionIndex} to ${targetIndex} after section refresh`);
          ...
          console.log('✅ Phase 2 restored from backend - skipping Section 1 generation');
          return true; // Successfully restored, don't generate Section 1
        } else {
          console.log('📂 No existing Phase 2 progress in backend, will check localStorage or generate Section 1');
    ...
    const initPhase2 = async () => {
      ...
      if (sections.length > 0 && currentSection !== null) {
        console.log('✅ Sections already loaded from restore - skipping Section 1 generation', {
          sectionsCount: sections.length,
          currentSection,
          currentQuestionIndex,
          restoredIndex: restoredQuestionIndexRef.current
        ...
        if (restoredQuestionIndexRef.current !== null && currentQuestionIndex !== restoredQuestionIndexRef.current) {
          console.log(`📍 Restoring question index to ${restoredQuestionIndexRef.current} during init check`);
      ...
      if (hasBackendProgress) {
        console.log('✅ Phase 2 already has progress - restoration complete, skipping Section 1 generation');
        return; // Don't generate Section 1 if we restored from backend
      }

      // If no backend progress, proceed with initialization
      console.log('🚀 No backend progress found - starting fresh Phase 2 initialization...');
        ...
        if (savedSession) {
          try {
            ...
            if (parsed.backendSessionId) {
              ...
              console.log('✅ Found backendSessionId in localStorage:', actualSessionId);
            }
          } catch (e) {
        ...
        if (!actualSessionId) {
          ...
          try {
            ...
            console.log('🔍 Workspace lookup:', {
              ...
              fromLocalStorage: workspaceStr ? JSON.parse(workspaceStr)?.id : null,
              finalWorkspaceId: workspaceId,
            });
            
            if (workspaceId) {
              console.log('🔨 Creating new backend session with workspace:', workspaceId);
              const newSession = await createAvaSession(workspaceId, `AVA Profile - ${userName} (Phase 2 Recovery)`);
              ...
              if (newSessionId) {
                ...
                throw new Error('No session ID returned from new session creation');
              }
              ...
              if (savedSession) {
                try {
                  ...
                  localStorage.setItem('ava-chat-session', JSON.stringify(parsed));
                  console.log('💾 Updated localStorage with new backendSessionId');
                } catch (e) {
                  console.error('❌ Failed to update localStorage:', e);
                }
              }
            } else {
              throw new Error('No workspace ID found. Please ensure you have a workspace selected.');
            }
          } catch (error: any) {
            console.error('❌ Failed to create backend session:', error);
            setInitError(`Failed to create backend session: ${error.message}. Please restart from Phase 1.`);
            console.groupEnd();
            return;
          }
        }
      ...
      if (!actualSessionId) {
        console.error('❌ CRITICAL: Still no backendSessionId after all recovery attempts!');
        setInitError('Backend session ID is missing. Please restart from Phase 1.');
        return;
      }
      ...
      try {
        ...
        try {
          ...
          console.log(`✅ Retrieved ${backendAnswers.length} Phase 1 answers from backend`);
          ...
          // Load ALL Phase 1 messages from backend first, then localStorage fallback
          if (!hasLoadedPhase1Messages.current && currentWorkspace?.id && phase1IntroMessages.length === 0 && actualSessionId) {
            hasLoadedPhase1Messages.current = true;
            try {
              // Try backend first (source of truth)
              const backendHistory = await getConversationHistory(actualSessionId);
              if (backendHistory.messages && backendHistory.messages.length > 0) {
                // Deduplicate messages before setting
                const uniqueMessages = backendHistory.messages.filter((msg: any, index: number, self: any[]) => {
                  const firstIndex = self.findIndex((m: any) => 
                    m.id === msg.id || (m.content === msg.content && m.role === msg.role)
                  );
                  return index === firstIndex;
                });
                setPhase1IntroMessages(uniqueMessages);
                console.log(`✅ Loaded ${uniqueMessages.length} unique Phase 1 messages from backend during initialization (deduplicated from ${backendHistory.messages.length})`);
              } else {
                // Fallback to localStorage
                ...
                if (phase1State) {
                  ...
                  if (parsed.messages && Array.isArray(parsed.messages)) {
                    // Deduplicate messages before setting
                    const uniqueMessages = parsed.messages.filter((msg: any, index: number, self: any[]) => {
                      const firstIndex = self.findIndex((m: any) => 
                        m.id === msg.id || (m.content === msg.content && m.role === msg.role)
                      );
                      return index === firstIndex;
                    ...
                    // Load ALL messages including intro, name submission, video, questions, and answers
                    setPhase1IntroMessages(uniqueMessages);
                    console.log(`✅ Loaded ${uniqueMessages.length} unique Phase 1 messages from localStorage during initialization (deduplicated from ${parsed.messages.length})`);
                  }
                }
              }
            } catch (error) {
              console.warn('⚠️ Failed to load Phase 1 messages during initialization:', error);
              hasLoadedPhase1Messages.current = false; // Reset on error so we can retry
            }
          }
          ...
          console.error('❌ Failed to fetch Phase 1 answers from backend:', fetchError);
        ...
        console.log('📡 Step 1: Checking session status before starting Phase 2...');
        ...
        const isAlreadyCompleted = sessionDetails?.currentPhase === 'completed' || sessionDetails?.currentPhase === 'complete';
        
        if (isAlreadyCompleted) {
          console.log(`✅ Session is already ${sessionDetails?.currentPhase} - skipping startPhase2 call`);
          console.log('🔄 Session is completed - attempting to restore all sections...');
          const hasBackendProgress = await checkBackendProgress();
          if (hasBackendProgress) {
            console.log('✅ Successfully restored completed Phase 2 from backend');
            hasInitialized.current = true;
            actualSessionIdRef.current = actualSessionId;
            return; // Exit initialization - sections already restored
          } else {
            console.warn('⚠️ Could not restore completed Phase 2 from backend - sections may be lost');
        ...
        if (!isAlreadyCompleted && !isAlreadyInPhase2) {
          ...
          const forceReinit = sessionStorage.getItem('phase2-force-reinit') === 'true';
          if (forceReinit) {
            console.log('🔄 Force re-initialization detected - calling startPhase2 API again');
          ...
          console.log('✅ Session is already in Phase 2 - skipping startPhase2 call, proceeding with section generation');
        }
        ...
        if (startResponse) {
          try {
            ...
            if (data?.message) {
              ...
              sessionStorage.setItem('phase2-chunks-count', String(messageContent.length));
              ...
              for (let idx = 0; idx < messageContent.length; idx++) {
                ...
                setPhase1IntroMessages(prev => {
                  ...
                  const exists = prev.some(m => m.id === introMessage.id || (m.content === msgHtml && m.role === 'ava'));
                  if (!exists) {
                    sessionStorage.setItem('phase2-chunks-displayed', String(idx + 1));
              ...
              console.log(`✅ Added ${messageContent.length} intro messages from startPhase2 response (displayed progressively)`);
              ...
              setPhase1IntroMessages(prev => {
                const exists = prev.some(m => m.content === messageContent && m.role === 'ava');
                if (!exists) {
                  console.log('✅ Added intro message from startPhase2 response');
                  return [...prev, introMessage];
                }
                return prev;
              });
        ...
        try {
          ...
          if (sessionDetails?.message && !forceReinit) {
            ...
            if (Array.isArray(messageContent)) {
              ...
              console.log(`✅ Added ${messageContent.length} intro messages from getSessionDetails response (displayed progressively)`);
            } else if (typeof messageContent === 'string') {
              // Single message string (backward compatibility)
              const introMessage = {
                id: `phase2-intro-${Date.now()}`,
              ...
              setPhase1IntroMessages(prev => {
                const exists = prev.some(m => m.content === messageContent && m.role === 'ava');
                if (!exists) {
                  console.log('✅ Added intro message from getSessionDetails response');
                  return [...prev, introMessage];
                }
                return prev;
              });
          ...
          const validPhases = ['phase2', 'phase_2', 'Phase2', 'PHASE2', 'transition'];
          if (!validPhases.includes(sessionDetails.currentPhase)) {
            console.warn('⚠️ Session not in phase2 state:', sessionDetails.currentPhase);
            // If phase is close to phase2, proceed anyway (backend may auto-transition)
            if (sessionDetails.currentPhase && sessionDetails.currentPhase.toLowerCase().includes('phase')) {
              console.log('✅ Phase appears valid, proceeding despite format mismatch');
            } else {
              setInitError(`Session is not ready for Phase 2. Current phase: ${sessionDetails.currentPhase}. The backend may need to transition the session. Try clicking "Retry Phase 2".`);
              console.groupEnd();
              return;
            }
          }
          ...
          console.warn('⚠️ Could not verify session state, proceeding anyway:', verifyError);
        }
        ...
        const savedIndex = restoredQuestionIndexRef.current !== null 
          ? restoredQuestionIndexRef.current 
          : currentQuestionIndex;
        ...
        if (restoredQuestionIndexRef.current !== null) {
          ...
          setCurrentQuestionIndex(restoredQuestionIndexRef.current);
          console.log(`📍 Preserving restored question index ${restoredQuestionIndexRef.current} for Section 1`);
          ...
          setCurrentQuestionIndex(0);
          console.log(`📍 Starting fresh - setting question index to 0 for Section 1`);
        } else {
          // Preserve existing question index when regenerating existing section
          setCurrentQuestionIndex(savedIndex);
          console.log(`📍 Preserving existing question index ${savedIndex} for Section 1`);
        }
        ...
        try {
          ...
          if (answersCount === 0) {
            setInitError(`No Phase 1 answers found! Please complete Phase 1 first. Found ${answersCount} answers.`);
          } else if (answersCount < 27) {
            setInitError(`Incomplete Phase 1 data (${answersCount}/27 answers). Backend may not have all answers saved. Please restart from Phase 1.`);
          } else {
            setInitError(`Section generation failed: ${error.message}. This is likely a backend issue. Check: 1) AI service (Gemini) is configured, 2) Phase 1 answers are saved in database (should have 27), 3) Backend logs for details. The error suggests the backend cannot generate AI responses - check your AI API key and service configuration.`);
          }
          
          // Don't fail completely - show a fallback UI
          console.log('⚠️ Proceeding with fallback - showing Phase 2 UI without initial section');
          
          // Set a dummy section so user can at least see Phase 2 interface
          const fallbackSection: Section = {
            sectionNumber: 1,
            sectionTitle: 'Section 1',
            sectionIntro: '⚠️ Generation failed. Click "Regenerate Section" below or check backend logs.',
            questions: [],
            isComplete: false,
          };
        ...
        console.log('🔍 RAW sectionResponse:', JSON.stringify(sectionResponse, null, 2));
        const sectionPayload = sectionResponse?.data ?? sectionResponse;
        console.log('🔍 EXTRACTED sectionPayload:', JSON.stringify(sectionPayload, null, 2));

        // Parse questions from response
      const questions: QuestionResponse[] = (sectionPayload?.questions || []).map((q: any) => ({
        id: q.id,
        questionId: q.questionId,
        questionText: q.questionText,
        aiGeneratedAnswer: q.aiGeneratedAnswer,
        ...
        console.log('📝 NEW SECTION TO ADD:', JSON.stringify(newSection, null, 2));
        setSections([newSection]);
        console.log('✅ Step 2: Section 1 added to state via setSections');
        console.log('✅ setSections([newSection]) called - React should re-render now');
        ...
        if (shouldHidePhase1Messages) {
          ...
          setTimeout(() => {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                setShowPhase1Messages(true);
                console.log('📜 Showing Phase 1 messages now that Phase 2 section is ready - user can scroll up to see them');
              });
            });
        ...
        if (!sectionPayload?.allReady) {
          console.log('⏳ Step 3: Starting status polling (GET /ava-sessions/:id/section/1/status)');
    ...
    console.log('⏱️ Scheduling initPhase2 to run in 1000ms to allow restore to complete...');
  ...
  const handleGenerateSection = async (sectionNumber: number): Promise<void> => {
    ...
    const existingSection = sections.find(s => s.sectionNumber === sectionNumber);
    const isExistingSection = !!existingSection;
    const previousSection = currentSection; // Store previous section before we change it
    const shouldPreserveIndex = isExistingSection && sectionNumber === previousSection;
    ...
    if (isExistingSection && existingSection.questions.length > 0) {
      ...
      if (shouldPreserveIndex && currentQuestionIndex >= 0) {
        ...
        const firstUnapprovedIndex = existingSection.questions.findIndex(q => !q.isApproved);
        if (firstUnapprovedIndex >= 0) {
      ...
      console.log(`📡 POST /ava-sessions/${currentSessionId}/generate-section/${sectionNumber}`);
      console.log(`🔄 isGenerating=true, isTransitioningSection=${isTransitioningSection}, isExistingSection=${isExistingSection}, preservedIndex=${preservedIndex}`);
    ...
    try {
      await savePhase2Progress(currentSessionId, sectionNumber, newQuestionIndex);
      console.log(`💾 Progress saved when navigating to Section ${sectionNumber}`);
    } catch (error) {
    ...
    try {
      ...
      const newSection: Section = {
        ...
        sectionTitle: sectionPayload?.sectionTitle || `Section ${sectionNumber}`,
        sectionIntro: sectionPayload?.sectionIntro || '',
        questions,
        isComplete: false,
      };
      ...
      const hasReadyQuestion = questions.some(q => q.status === 'ready' || q.aiGeneratedAnswer);
      ...
      setSections(prev => {
        ...
        if (existing && existing.isComplete) {
          ...
          console.log(`🔄 Section ${sectionNumber} already completed, preserving approval state for review`);
          updatedSection = {
            ...newSection,
            isComplete: true, // Keep it marked as complete
            questions: newSection.questions.map(newQ => {
              const existingQ = existing.questions.find(q => 
              ...
              return {
                ...
                userEditedAnswer: existingQ?.userEditedAnswer || newQ.userEditedAnswer,
              };
            }),
          };
        } else if (existing) {
          // Section exists but not complete - merge with existing to preserve approval states
          console.log(`🔄 Section ${sectionNumber} exists but not complete, merging with existing data`);
          updatedSection = {
            ...newSection,
            questions: newSection.questions.map(newQ => {
              const existingQ = existing.questions.find(q => 
        ...
        if (isExistingSection && updatedSection.questions.length > 0 && sectionNumber !== previousSection) {
          // Navigating to a different existing section - show first unapproved question
          const firstUnapprovedIndex = updatedSection.questions.findIndex(q => !q.isApproved);
          if (firstUnapprovedIndex >= 0) {
            // Show up to and including the first unapproved question
            setCurrentQuestionIndex(firstUnapprovedIndex);
            console.log(`📍 Navigating to section ${sectionNumber} from ${previousSection}, showing first unapproved question at index ${firstUnapprovedIndex}`);
          } else {
            // All approved, show all questions
            setCurrentQuestionIndex(updatedSection.questions.length - 1);
            console.log(`📍 All questions approved in section ${sectionNumber}, showing all (index ${updatedSection.questions.length - 1})`);
          }
        } else if (isExistingSection && sectionNumber === previousSection) {
          // Same section - index was already preserved before API call, keep it
          console.log(`📍 Staying on section ${sectionNumber}, preserving currentQuestionIndex ${preservedIndex}`);
        }
        
        if (existing && existing.isComplete) {
          return prev.map(s => s.sectionNumber === sectionNumber ? updatedSection : s);
        } else if (existing) {
          return prev.map(s => s.sectionNumber === sectionNumber ? updatedSection : s);
        }
        return [...prev, updatedSection].sort((a, b) => a.sectionNumber - b.sectionNumber);
      });

      // If we have at least one ready question, clear transition state
      // Use functional update to avoid stale closure issues
      if (hasReadyQuestion) {
        console.log(`✅ Section ${sectionNumber} has ready questions, clearing transition state if needed`);
        setIsTransitioningSection(prev => {
          if (prev) {
            console.log(`✅ Clearing transition state for Section ${sectionNumber}`);
            return false;
          }
          return prev;
        });
  ...
  const startPollingStatus = (sessionId: string, sectionNumber: number) => {
    ...
    console.log(`🔄 Polling Section ${sectionNumber} status (attempt ${pollCount + 1})...`);

    pollIntervalRef.current = setInterval(async () => {
      const currentSessionId = sessionId || actualSessionIdRef.current || backendSessionId;
      if (!currentSessionId) {
      ...
      try {
        console.log(`📡 GET /ava-sessions/${currentSessionId}/section/${sectionNumber}/status`);
        const statusResponse = await getSectionStatus(currentSessionId, sectionNumber);
        ...
        const newQuestions: QuestionResponse[] = (status?.questions || []).map((q: any) => ({
          id: q.id,
          questionId: q.questionId,
          questionText: q.questionText,
          aiGeneratedAnswer: q.aiGeneratedAnswer,
        ...
        setSections(prev => {
          const sectionData = prev.find(s => s.sectionNumber === sectionNumber);
          if (!sectionData) return prev;
          
          // Check if section is already complete - if so, stop polling
          const allQuestionsApproved = sectionData.questions.every(q => q.isApproved);
          if (allQuestionsApproved && sectionData.questions.length > 0) {
            console.log(`✅ Section ${sectionNumber} already complete - stopping polling`);
            if (pollIntervalRef.current) {
              clearInterval(pollIntervalRef.current);
              pollIntervalRef.current = null;
            }
          ...
          const mergedQuestions = newQuestions.map(newQ => {
            const existingQ = sectionData.questions.find(q => 
              (q.id && q.id === newQ.id) || (q.questionId === newQ.questionId)
            );
          ...
          const hasReadyQuestion = mergedQuestions.some(q => q.status === 'ready' || q.aiGeneratedAnswer);
          const previousHasReadyQuestion = sectionData.questions.some(q => q.status === 'ready' || q.aiGeneratedAnswer);
          
          // If this is the first ready question and we're transitioning, clear transition state
          if (hasReadyQuestion && !previousHasReadyQuestion && sectionNumber === currentSection) {
            console.log(`✅ First ready question detected for Section ${sectionNumber}, clearing transition state`);
            setIsTransitioningSection(false);
          }
          ...
          console.log(`📊 Polling update for Section ${sectionNumber}:`, {
            ...
            questionStatuses: mergedQuestions.map(q => ({ id: q.questionId, status: q.status, isApproved: q.isApproved }))
          });
        ...
        if (status?.allReady) {
          ...
          console.log(`✅ Section ${sectionNumber}: All ${totalQuestions} questions ready!`);
          console.log(`📋 Complete question list:`, status?.questions?.map((q: any, idx: number) => ({
            index: idx,
            questionId: q.questionId,
        ...
        console.error(`❌ Failed to poll Section ${sectionNumber} status:`, error);
  ...
  const handleConfirmQuestion = async (question: QuestionResponse, section: Section) => {
    const currentSessionId = actualSessionIdRef.current || backendSessionId;
    if (!currentSessionId || !question.id) return;
    console.log(`✅ Confirming question ${question.questionId} (responseId=${question.id}) in section ${section.sectionNumber}`);
    
    // Once the user confirms a question, rely on live state instead of restored indices
    restoredQuestionIndexRef.current = null;
    hasRestoredState.current = false;
    
    const currentQuestionIdx = section.questions.findIndex(q => q.id === question.id);
    ...
    let apiResult = null;
    try {
      console.log(`📞 API Call 1: Confirming individual question via confirm-phase2-question...`);
      apiResult = await confirmSectionResponse(currentSessionId, section.sectionNumber, question.id);
      ...
      toast({
        title: "Error",
        description: err?.message || "Failed to confirm question. Please try again.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (isLastQuestion) {
      try {
        console.log(`📞 API Call 2: This is the LAST question - confirming entire section`);
        const sectionConfirmResult = await confirmSection(currentSessionId, section.sectionNumber);
        ...
        console.warn('⚠️ Failed to confirm section (non-critical):', err?.message);
      }
    }
    ...
    const willCompleteSection = isLastQuestion && 
      section.questions.filter(q => q.id !== confirmedQuestionId).every(q => q.isApproved);
    
    console.log('🔍 Pre-confirmation check:', {
      isLastQuestion,
      totalQuestions: section.questions.length,
      alreadyApprovedCount: section.questions.filter(q => q.isApproved).length,
      willCompleteSection
    });
    
    setSections(prev => {
      const currentSection = prev.find(s => s.sectionNumber === confirmedSectionNumber);
      if (!currentSection) return prev;
      
      const updatedSection = {
        ...currentSection,
        questions: currentSection.questions.map(q => {
          const isMatch = (q.id === confirmedQuestionId) || (q.questionId === confirmedQuestionStringId);
          if (isMatch) {
          ...
          if (q.isApproved) {
            console.log('🔒 Preserving approved state for question:', q.questionId);
          }
          return q;
        }),
      };
      
      const currentQIndex = updatedSection.questions.findIndex(q => 
        q.id === confirmedQuestionId || q.questionId === confirmedQuestionStringId
      );
      ...
      console.log(`🔍 Question index check:`, {
        ...
        confirmedQuestionId,
        allQuestionIds: updatedSection.questions.map(q => ({ id: q.id, questionId: q.questionId, status: q.status, isApproved: q.isApproved })),
        isGenerating,
        hasGeneratingQuestions: updatedSection.questions.some(q => q.status === 'generating')
      });
      ...
      if (currentQIndex < updatedSection.questions.length - 1) {
        console.log(`➡️ Moving to next question (index ${currentQIndex + 1} of ${updatedSection.questions.length} loaded questions)`);
    ...
    if (willCompleteSection) {
      console.log(`✅ Section ${confirmedSectionNumber} will be complete after this confirmation`);
      
      setTimeout(() => {
        console.log(`🔄 Triggering section transition from ${confirmedSectionNumber} to next section`);
        
        setIsTransitioningSection(true);
        
        setSectionsList(prevList => prevList.map(s => 
          s.sectionNumber === confirmedSectionNumber ? { ...s, isApproved: true } : s
        ));
        
        setSections(prev => prev.map(s =>
          s.sectionNumber === confirmedSectionNumber ? { ...s, isComplete: true } : s
        ));
        
        setCurrentQuestionIndex(0);
        
        const nextSectionNumber = apiResult?.nextSection || (confirmedSectionNumber + 1);
        ...
        if (nextSectionNumber <= 21) {
          ...
          handleGenerateSection(nextSectionNumber).catch((error) => {
            console.error(`❌ Failed to generate Section ${nextSectionNumber}:`, error);
            setIsTransitioningSection(false);
          });
        } else {
    ...
    if (apiResult?.sectionCompleted) {
      console.log(`✅ Section ${section.sectionNumber} questions all approved (from API).`);
    }
  };

  const handleRegenerateQuestion = async (question: QuestionResponse, section: Section) => {
    const currentSessionId = actualSessionIdRef.current || backendSessionId;
    if (!currentSessionId || !question.id) return;
    ...
    try {
      const result = await regeneratePhase2Question(currentSessionId, question.id);
      console.log('✅ Question regeneration API response:', result);
      
      const regeneratedAnswer = result?.data?.aiGeneratedAnswer || result?.aiGeneratedAnswer;
      console.log('✅ New regenerated answer:', regeneratedAnswer ? `${regeneratedAnswer.substring(0, 100)}...` : 'NONE');
      
      if (regeneratedAnswer) {
        console.log('✅ Updating question with new answer for question ID:', question.id);
        ...
        setSections(prev => {
          ...
          const updatedSection = updatedSections.find(s => s.sectionNumber === section.sectionNumber);
          const updatedQuestion = updatedSection?.questions.find(q => q.id === question.id);
          console.log('✅ State after update - Question has answer:', !!updatedQuestion?.aiGeneratedAnswer);
          console.log('✅ State after update - Answer preview:', updatedQuestion?.aiGeneratedAnswer?.substring(0, 100));
  ...
  const handleSaveEdit = async (sectionNumber: number, responseId: string, questionId: string) => {
    const currentSessionId = actualSessionIdRef.current || backendSessionId;
    if (!currentSessionId || !editingQuestionId || !responseId) {
      console.error('❌ Cannot save edit: Missing required data', { currentSessionId, editingQuestionId, responseId });
      toast({
        title: "Error",
        description: "Backend session ID is missing. Please refresh the page.",
        variant: "destructive",
    ...
    console.log(`💾 Saving edit for Section ${sectionNumber}, Question ${questionId}`);
    
    setIsSavingEdit(true);
    try {
      await updateSectionResponse(currentSessionId, sectionNumber, responseId, editedContent);
  ...
  return (
    <>
      ...
      <div ref={messagesContainerRef} className="px-[20%] pb-32 pt-2" style={{ scrollBehavior: 'smooth' }}>
        <div className="w-full space-y-6" style={{ minHeight: '100%' }}>
          ...
          {showPhase1History && phase1IntroMessages.length > 0 && showPhase1Messages && sessionStorage.getItem('ava-phase2-hide-phase1-messages') !== 'true' && (
            <div className="space-y-4 mb-6 pt-6">
              {phase1IntroMessages.filter((msg, index, self) => {
                ...
                <div key={`${msg.id}-${index}`}>
                  {msg.role === 'ava' ? (
                    <div className="flex gap-3 items-start">
                      ...
                      />
                      <div className="w-[70%]">
                        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl rounded-tl-sm px-4 py-3" style={index === 0 ? { marginTop: '58px' } : undefined}>
                          ...
                          <div className={`text-[15px] leading-relaxed text-slate-200 ${(() => {
                            ...
                            const htmlTagRegex = /<[^>]+>/;
                            const isHTML = typeof msg.content === 'string' && htmlTagRegex.test(msg.content);
                          ...
                          {msg.videoUrl && (
                            <div className="mt-4">
                              <div className="relative w-full bg-black rounded-lg overflow-hidden" style={{ paddingTop: '56.25%' }}>
                                <iframe
                                  src={(() => {
                                    const match = msg.videoUrl.match(/vimeo\.com\/(\d+)(?:\/([a-zA-Z0-9]+))?/);
                                    if (!match) return '';
                          ...
                          {msg.examples && msg.examples.length > 0 && !isStarted && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="text-xs font-medium text-vox-pink mb-2">Examples:</div>
                              <div className="space-y-2">
                                {msg.examples.map((example, idx) => (
                                  <div key={idx} className="text-sm text-slate-300 bg-slate-800/50 border border-slate-700/50 px-3 py-2 rounded-lg">
                                    {example}
                                  </div>
                                ))}
                              </div>
          ...
          {showPhase1History && phase1History.length > 0 && phase1IntroMessages.length === 0 && sessionStorage.getItem('ava-phase2-hide-phase1-messages') !== 'true' && (
            <div className="space-y-6 mb-8">
              {phase1History.map((item, idx) => {
                const sectionTitle = item.sectionTitle || (item.sectionId ? sectionTitleMap[item.sectionId] : null);
                return (
                  <div key={item.questionId || idx} className="space-y-4">
                    {/* Question - AVA Message (Left Side) */}
                    <div className="flex gap-3 items-start">
                      {/* AVA Avatar */}
                      <img 
                        src={avaAvatar} 
          ...
          <div ref={phase2StartRef} className={`flex justify-center pt-4 mb-6 ${(phase1IntroMessages.length > 0 || phase1History.length > 0) && !isStarted ? '' : 'pt-6'}`}>
          ...
          {initError && !isStarted && (
            <div className="flex flex-col items-center gap-4">
              <div className="px-6 py-3 bg-destructive/10 border border-destructive/20 rounded-xl shadow-sm max-w-2xl">
                ...
                <p className="text-xs text-center text-slate-400 mt-2">
                  Check browser console for detailed error logs. The backend may be experiencing issues, but you can try generating sections manually.
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={async () => {
                    console.log('🔄 Attempting to generate Section 1 manually...');
                    setInitError(null);
                    setIsStarted(true);
                    setIsGenerating(true);
                    try {
                      ...
                      setInitError(`Section generation still failing: ${error.message}. Please check backend logs.`);
                      setIsGenerating(false);
                    }
                  }}
          ...
          {(() => {
            if (sections.length > 0) {
              console.log(`🎨 RENDERING ${sections.length} SECTIONS IN UI`, sections);
            }
            return null;
          })()}

          {/* Show loader when transitioning to a section that doesn't exist yet */}
          {isTransitioningSection && currentSection && !sections.find(s => s.sectionNumber === currentSection) && (
          ...
          {sections.map((section) => {
            ...
            console.log(`🎨 Rendering section ${section.sectionNumber} with ${section.questions.length} questions`);
            
            // Get section metadata from sectionsList for title and intro
            const sectionMeta = sectionsList.find(s => s.sectionNumber === section.sectionNumber);
            ...
            const effectiveIndex = restoredQuestionIndexRef.current !== null && isCurrentSection
              ? restoredQuestionIndexRef.current
              : currentQuestionIndex;
              ...
              >
                ...
                <div className="flex justify-center">
                  <div className="px-6 py-3 bg-gradient-to-r from-vox-pink/10 via-vox-purple/10 to-vox-orange/10 border border-vox-pink/20 rounded-xl shadow-sm max-w-2xl">
                    <p className="text-sm text-center font-medium">
                      <strong className="font-semibold">Section {section.sectionNumber}: {sectionTitle}</strong>
                      {sectionIntro && (
                        <span className="block mt-2 text-xs text-muted-foreground font-normal">{sectionIntro}</span>
                      )}
                    </p>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const hasReadyQuestions = questionsToShow.some(q => q.status === 'ready' || q.aiGeneratedAnswer);
                  const isCurrentSection = currentSection === section.sectionNumber;
                  ...
                  const shouldShowLoader = isCurrentSection && 
                                          !hasReadyQuestions && 
                                          (isTransitioningSection || isGenerating || !hasQuestions);
                  
                  if (shouldShowLoader) {
                    console.log(`📍 Showing loader for Section ${section.sectionNumber}:`, {
                      isGenerating,
                    ...
                    console.log(`✅ Loader hidden for Section ${section.sectionNumber}: first question appeared`, {
                      questionsToShowCount: questionsToShow.length,
                ...
                {questionsToShow.map((question, qIndex) => {
                  ...
                  if (qIndex === 0) {
                    console.log(`📝 First question appeared for Section ${section.sectionNumber} - loader should hide`);
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex gap-3 items-start">
                        <div className="w-[70%]">
                          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl rounded-tl-sm px-4 py-3">
                            <div className="text-xs text-slate-200 mb-1" style={{ fontWeight: 700}}>
                              Question {section.sectionNumber}.{qIndex + 1}
                            </div>
                            <p className="text-[15px] leading-relaxed text-slate-200 whitespace-pre-wrap" style={{ fontWeight: 600 }}>
                              {safeQuestion.questionText.replace(/^Question \d+\s*/i, '')}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      {safeQuestion.status === 'ready' && (
                        <div className="space-y-3">
                          ...
                          <div className="flex gap-3 items-start justify-end">
                            ...
                            <div className={`${isEditing ? 'px-0 py-2' : 'bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-2xl rounded-tr-sm px-4 py-3'} w-full transition-all duration-300 ${isEditing ? '' : 'animate-ai-answer-in'}`}>
                            {isEditing ? (
                                  <Textarea
                                    value={editedContent}
                              ...
                              <div className="flex gap-3 items-center w-full justify-between mt-3 pt-3 border-t border-gray-100">
                                <div className="flex gap-2.5 items-center">
                                  ...
                                  {(!safeQuestion.isApproved) && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => handleConfirmQuestion(question, section)}
                                  ...
                                  {isEditing ? (
                                    <div className="flex gap-2.5 items-center">
                                      ...
                                      <Button
                                        size="sm"
                                        onClick={() => handleSaveEdit(section.sectionNumber, question.id!, question.questionId)}
                                    ...
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => handleEdit(question.questionId, displayAnswer)}
                                    ...
                                    >
                                      ...
                                      <span className="relative z-10">Edit Answer</span>
                                    </Button>
                                  )}
                                </div>
                                ...
                                {(!safeQuestion.isApproved) && (
                                  <Button
                                    ...
                                    onClick={() => handleRegenerateQuestion(question, section)}
                                  ...
                                  >
                                    {regeneratingQuestionId === question.id ? (
                                      ...
                                      <>
                                        ...
                                        <span className="relative z-10">AI Regenerate</span>
                                      </>
                                    )}
              ...
              {section.isComplete && (
                <>
                  ...
                  {(() => {
                    ...
                    const nextSection = sections.find(s => s.sectionNumber === nextSectionNumber);
                    // Show loader if:
                    ...
                    const shouldShowLoader = currentSection === nextSectionNumber && 
                                           (isTransitioningSection || isGenerating) &&
                                           (!nextSection || 
                                            nextSection.questions.length === 0 || 
                                            !nextSection.questions.some(q => q.status === 'ready' || q.aiGeneratedAnswer));
            ...
            <div className="flex flex-col items-center justify-center gap-6 py-12 animate-fade-in">
              ...
              <div className="bg-white border-2 border-vox-pink/20 rounded-xl shadow-xl p-8 max-w-2xl w-full space-y-6">
                <div className="space-y-3">
                  <Button
                    onClick={async () => {
                      if (_externalExportPDF) {
                        ...
                        try {
                          const { exportProfilePDF } = await import('@/lib/ava-api');
                          const blob = await exportProfilePDF(currentSessionId);
                          const url = URL.createObjectURL(blob);
                          const a = document.createElement('a');
                          a.href = url;
                          a.download = `AVA-Profile-${userName}-${new Date().toISOString().split('T')[0]}.pdf`;
                          a.click();
                  ...
                  {_onStartOver && (
                    <>
                      ...
                      <AlertDialog open={showCreateNewProfileDialog} onOpenChange={setShowCreateNewProfileDialog}>
                        <AlertDialogContent className="bg-background">
                          <AlertDialogHeader>
                            <AlertDialogTitle>Start New Profile?</AlertDialogTitle>
                            <AlertDialogDescription>
                ...
                <div className="pt-6 border-t space-y-3">
                  <h3 className="font-semibold text-sm text-muted-foreground">Next Steps</h3>
                  <ul className="space-y-2 text-sm text-left">
                    <li className="flex items-start gap-2">
                      <span className="text-vox-pink">✓</span>
                      <span>Use this profile to craft hyper-relevant marketing messages</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-vox-pink">✓</span>
                      <span>Share with your team to align on ideal client vision</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-vox-pink">✓</span>
                      <span>Apply insights to all VoxBox Amplifiers for consistent messaging</span>
                    </li>
                  </ul>
                </div>
              </div>
      ...
      <AlertDialog open={showNewSessionDialog} onOpenChange={setShowNewSessionDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to create a new session? This will reset your current progress and start fresh.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
              ...
              onClick={async () => {
                ...
                if (currentWorkspace?.id) {
                  ...
                  Object.keys(localStorage).forEach(key => {
                    if (key.startsWith(`ava-phase1-state-${currentWorkspace.id}-`) || 
                  ...
                  hasRestoredState.current = false;
                  hasRestoredQuestionScroll.current = false;
                  console.log('🧹 Cleared all restored state references for new session');
                }
